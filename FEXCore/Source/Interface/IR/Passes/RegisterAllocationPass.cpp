...
bool ConstrainedRAPass::Run(IREmitter* IREmit) {
  ...
  auto IsInRegisterFile = [this, &IR, &Map, &IsOld, &SSAToReg](auto Old) {
    ...
    auto Class = &Classes[Reg.Class == GPRPairClass ? GPRClass : Reg.Class];
  ...
  auto SpillReg = [&IR, &IREmit, &SpillSlotCount, &SpillSlots, &SSAToReg, &FreeReg, &Map, &Unmap, &IsOld,
    ...
    uint8_t BestReg = ~0;
    unsigned ClassCount = Class->Count;

    // If we're spilling make room for a register pair, make sure we're spilling
    // registers that could possibly used as part of the pair so we actually
    // make progress (and so that the shuffle code works).
    if (Pair) {
      LOGMAN_THROW_AA_FMT(PAIR_COUNT <= ClassCount, "By construction");
      ClassCount = PAIR_COUNT;
    }
  ...
  auto AssignReg =
    [this, &IR, IREmit, &SpillReg, &ClassSize, &Remap, &FreeReg, &SetReg, &Map, &AvailableMask](OrderedNode* CodeNode, IROp_Header* Pivot) {
    ...
    //
    // There is one subtlety though: if we are allocating a pair, we need at
    // least 1 free register that could be a pair, else we could end up trying
    // to allocate a pair when the only free 2 regs are outside the pair region,
    // and the pair region is made of all pairs (so nothing to shuffle).
    // As long as we have 1 free register in the pair region, its neighbour is a
    // scalar that can be evicted outside the pair region to make room for the
    // pair inside the pair region.
    //
    ...
    while (std::popcount(Class->Available) < ClassSize(OrigClassType) || (Pair && !(Class->Available & PAIR_BITS))) {
