...
namespace {
  ...
  struct RegisterClass {
    ...
    // Allocated base registers. Similar to ~Available except for pairs and SRA.
    uint32_t Allocated;
  };
...
bool ConstrainedRAPass::Run(IREmitter* IREmit) {
  ...
  auto AssignReg = [this, &IR, IREmit, &SpillReg, &Remap, &FreeReg, &SetReg, &Map, &AvailableMask, &PreferredReg, &PreferredEligible,
    ...
    if (!Available) {
      ...
      LOGMAN_THROW_AA_FMT(!(Class->Available & (1u << Blocked)), "Invariant");
  ...
  for (auto [BlockNode, BlockHeader] : IR.GetBlocks()) {
    // At the start of each block, all general registers are available.
    Classes[GPRClass].Available = (1u << Classes[GPRClass].Count) - 1;
    Classes[FPRClass].Available = (1u << Classes[FPRClass].Count) - 1;

    // Fixed registers are always allocated at block boundaries.
    Classes[GPRFixedClass].Available = 0;
    Classes[FPRFixedClass].Available = 0;
    ...
    //  - insert moves for tied operands (TODO)
    {
      ...
      while (1) {
        ...
        // Record preferred registers for SRA.
        if (auto [Node, Reg] = DecodeSRA(CodeNode); Node != nullptr) {
          PreferredReg[IR.GetID(Node).Value] = Reg;

          // If a register is stored before loaded, that store can be coalesced.
          // But if loaded before stored, the first store cannot be coalesced.
          // This means Available should be initially true if a store precedes
    ...
    for (auto [CodeNode, IROp] : IR.GetCode(BlockNode)) {
        ...
        if (Class->Allocated & RegBit) {
          ...
          if (Old != Node) {
            ...
            if (Reg.Class == FPRFixedClass) {
              auto Header = IR.GetOp<IROp_Header>(Old);
              Copy = IREmit->_VMov(16 /*Header->ElementSize*/, Map(Old));
            } else {
      ...
      foreach_valid_arg(IROp, _, Arg) {
        ...
        // XXX: what if shuffle code for pairs here
      ...
      foreach_valid_arg(IROp, i, Arg) {
        ...
        if (auto Remapped = SSAToNewSSA.at(Arg.ID().Value); Remapped != nullptr) {
      ...
      foreach_valid_arg(IROp, i, Arg) {
        ...
        // Remap again, in case AssignReg shuffled a source.
        auto Old = Unmap(IR.GetNode(Arg));
        const auto New = Map(Old);

        if (New != IR.GetNode(Arg)) {
