...
#define foreach_arg(IROp, index) for (auto index = 0; index < IR::GetRAArgs(IROp->Op); ++index)

#define foreach_valid_arg(IROp, index) foreach_arg(IROp, index) if (IsValidArg(IROp->Args[index]))
...
private:
  ...
  void SpillReg(RegisterClass* Class, IROp_Header* Exclude, bool Pair) {
    ...
    bool Spilled = SpillSlots.size() > 0 && SpillSlots[Value] != 0;
  ...
  void AssignReg(IROp_Header* IROp, OrderedNode* CodeNode, IROp_Header* Pivot) {
    ...
    if (!Available) {
      ...
      foreach_arg(Pivot, s) {
        OrderedNode* Clobber = IR->GetNode(Pivot->Args[s]);
